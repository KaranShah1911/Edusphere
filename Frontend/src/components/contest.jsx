...
class Contest extends Component {
  ...
  handleWalletClick = () => {
    this.setState((prevState) => ({
      isWalletClicked: !prevState.isWalletClicked,
    }));
  };
  ...
  render() {
    const { contests, loading, theme, walletAddress, isWalletClicked, dropdownOpen } = this.state;
    ...
    const settings = {
      ...
      autoplay: true,
      autoplaySpeed: 3000,
      slidesToShow: 1, // Show one contest at a time
      slidesToScroll: 1,
    ...
    return (
      <div className={`${isDark ? "bg-gradient-to-r from-black to-gray-700 text-gold" : "bg-gradient-to-r from-yellow-100 to-yellow-100 text-black"} transition-colors duration-300`}>
        ...
        <nav className="flex justify-between items-center p-6">
          ...
          <div className="flex items-center space-x-8">
            ...
            <button
              ...
              onClick={walletAddress ? this.handleWalletClick : this.connectWallet}
            >
              {walletAddress ? (
                <div>
                  {isWalletClicked ? (
        ...
        <div className={`container mx-auto my-1 p-8 rounded-lg shadow-lg ${isDark ? 'bg-gray-700 text-white' : 'bg-gradient-to-r from-yellow-100 to-white text-black'}`}>
            ...
            <Slider {...settings}>
              {contests.map((contest) => (
                <div className="card-wrapper" key={contest.id}>
                  <div className={` rounded-lg p-6 shadow-md ${isDark ? 'bg-gradient-to-r from-gold via-yellow-200 to-black text-black' : 'bg-gradient-to-r from-yellow-200 to-gray-100 text-black'}`}>
          ...
          <footer className={`text-center mt-28 p-4 my-10 rounded-lg ${isDark ? 'bg-gray-600' : 'bg-gray-200'}`}>
